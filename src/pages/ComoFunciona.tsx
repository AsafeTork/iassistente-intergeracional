...
export function ComoFunciona() {
  return (
    <div>
      ...
      <div className="grade">
        {CAMADAS.map((c) => (
          <article key={c.titulo} className="cartao">
            <p className="cartao-icone" aria-hidden="true">
              {c.icone}
            </p>
            <h3>{c.titulo}</h3>
            <p>{c.texto}</p>
      ...
      <section className="secao" aria-labelledby="tit-fluxo">
        <h2 id="tit-fluxo">Fluxo de uma ajuda (igual ao diagrama do projeto)</h2>
        <ol className="fluxo">
          <li>Você abre o Gov.br</li>
          <li>O app captura a tela (DOM)</li>
          <li>Tem dado sensível? <strong>Sim → limpa tudo</strong> / Não → segue</li>
          <li>A nuvem entende e manda o passo simplificado</li>
          <li>Você ouve e vê o destaque na tela — e conclui a tarefa</li>
        </ol>
        <p className="detalhe">
      ...
      <section className="secao faixa" aria-labelledby="tit-cache">
        <h2 id="tit-cache">Por que este protótipo é “offline-first”?</h2>
        <p>
        ...
        <Link to="/privacidade" className="botao botao-secundario">
