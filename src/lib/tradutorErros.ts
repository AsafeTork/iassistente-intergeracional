...
import { PASSOS_GOVBR, PASSOS_SUS, type Passo } from '../data/mock';

const FALLBACK =
  'Algo não saiu como esperado, e tudo bem. Vamos tentar de novo juntos, sem pressa.';

const TODOS_PASSOS: Passo[] = [...PASSOS_GOVBR, ...PASSOS_SUS];
...
const DICAS_POR_PASSO: Record<string, string> = {
  ...
  cartao: 'Confira se o cartão SUS tem 15 números, só números.',
};
...
export function traduzirErro(erroTecnico: string, passoId: string): string {
  const passo = TODOS_PASSOS.find((p) => p.id === passoId);
